...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        with autocast(enabled=self.config.train.mixed_precision):
            y_hat, ids_slice, phoneme_mask, mel_mask, (z_p, z_q, m_p, logs_p, m_q, logs_q), (logw, logw_), g = self.model(
        ...
        with autocast(enabled=self.config.train.mixed_precision):
            loss_mel = mel_spectrogram_loss(y_mel, y_hat_mel) * self.config.train.losses.mel_loss_weight

            loss_kl = kl_divergence_loss(z_p, logs_q, m_p, logs_p, phoneme_mask) * self.config.train.losses.kl_loss_weight

            loss_dur = duration_loss(logw, logw_, phoneme_lengths) * self.config.train.losses.duration_loss_weight
