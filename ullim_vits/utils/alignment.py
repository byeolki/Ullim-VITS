...
def maximum_path(neg_cent, mask):
    ...
    for i in range(neg_cent.shape[0]):
        path_i = maximum_path_c(neg_cent[i], t_t_max[i], t_s_max[i])
        path[i, :t_t_max[i], :t_s_max[i]] = path_i
...
def generate_path(duration, mask):
    ...
    for i in range(b):
        ...
        for j in range(min(dur_len, t_x)):
            start = 0 if j == 0 else int(cum_dur[j - 1].item())
            end = int(cum_dur[j].item())
            if end > start and end <= t_y:
